...
class Maze:
    def __init__(self, rows, columns):
        ...
        self.maze = np.empty((self.rows, self.columns), dtype=object)
        self.start = None
        self.end = None

    def findCellNeighbors(self, cell):
        neighbors = []
        ...
        return neighbors
    """
    This method uses recursive backtracking to generate a random maze
    """
    def recursiveBacktracking(self):
        ...
        while current_cell is not None:
            neighbors = self.findCellNeighbors(current_cell)
            if len(neighbors) > 0:
    ...
    def findPossiblePaths(self, cell):
        paths = []
    ...
    def visualizeMazeSolver(self, fps, scale):
        maze_img_arr = self.toImgArr()
        head = self.start
        current_cell = head
        answer = [head]
        while current_cell != self.end:
            ...
            if len(possible_paths) > 0:
                maze_img_arr[(current_cell.y * 3) + 1, (current_cell.x * 3) + 1, :] = np.array([255, 0, 0])
                if current_cell.parent_cell is not None:
                    maze_img_arr[(current_cell.y * 3) + 1 + (current_cell.parent_cell.y - current_cell.y), (current_cell.x * 3) + 1 + (current_cell.parent_cell.x - current_cell.x), :] = np.array([255, 0, 0])
                path = random.choice(possible_paths)
                maze_img_arr[(current_cell.y * 3) + 1 + (path.y - current_cell.y), (current_cell.x * 3) + 1 + (path.x - current_cell.x), :] = np.array([255, 0, 0])
                current_cell = path
                maze_img_arr[(current_cell.y * 3) + 1, (current_cell.x * 3) + 1: (current_cell.x * 3) + 2, :] = np.array([255, 0, 0])
                if current_cell.parent_cell is not None:
                    maze_img_arr[(current_cell.y * 3) + 1 + (current_cell.parent_cell.y - current_cell.y), (current_cell.x * 3) + 1 + (current_cell.parent_cell.x - current_cell.x), :] = np.array([255, 0, 0])

                answer.append(current_cell)
            else:
                maze_img_arr[(current_cell.y * 3) + 1: (current_cell.y * 3) + 2, (current_cell.x * 3) + 1: (current_cell.x * 3) + 2, :] = np.array([0, 255, 255])
                if current_cell.parent_cell is not None:
                    maze_img_arr[(current_cell.y * 3) + 1 + (current_cell.parent_cell.y - current_cell.y), (current_cell.x * 3) + 1 + (current_cell.parent_cell.x - current_cell.x), :] = np.array([0, 255, 255])
                for child in current_cell.children_cells:
                    maze_img_arr[(current_cell.y * 3) + 1 + (child.y - current_cell.y), (current_cell.x * 3) + 1 + (child.x - current_cell.x), :] = np.array([0, 255, 255])
    ...
    def illustrateRecursiveBacktracking(self, scale, fps):
        ...
        while current_cell is not None:
            ...
            if len(neighbors) > 0:
                ...
                # make new cell from random neighbor
                new_cell = random.choice(neighbors)
                ...
                # add new cell to current cells children list
                current_cell.children_cells.append(new_cell)

                # draw current cell
                current_cell_img = (current_cell.toImgArr().repeat(3).reshape((3, 3, 3))) * 255
                maze_img_arr[current_cell.y * 3: (current_cell.y * 3) + 3, current_cell.x * 3: (current_cell.x * 3) + 3] = current_cell_img

                # make new cell the current cell
                current_cell = new_cell
                ...
                # draw current cell as popped "visited"
                current_cell_img = (current_cell.toImgArr().repeat(3).reshape((3, 3, 3))) * 125
                maze_img_arr[current_cell.y * 3: (current_cell.y * 3) + 3, current_cell.x * 3: (current_cell.x * 3) + 3] = current_cell_img
                # backpropagate
                current_cell = current_cell.parent_cell
        ...
        maze_img_arr = (maze_img_arr == 125) * 255
    ...
    def toImgArr(self):
        ...
        for row in range(self.maze.shape[0]):
            for column in range(self.maze.shape[1]):
                x, y = (column * 3, row * 3)
                maze_img_arr[y: y + 3, x: x + 3] = self.maze[row, column].toImgArr()
